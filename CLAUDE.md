...
Обе дают падение PHP **без единого сообщения**: PHPUnit просто обрывается на середине прогона, код возврата 0. Найти можно только через `--debug` по последнему `Test Preparation Started`.

1. **`defined( 'ABSPATH' ) || exit;` в файлах классов.** В юнит-тестах `ABSPATH` не определён, и автозагрузка такого файла завершает процесс. Для файла, который только объявляет класс, эта заглушка ни от чего не защищает: прямой запрос к нему ничего не выполнит. Держим её только в точках входа — `ozon-delivery-for-woocommerce.php` (там `ABSPATH`) и `uninstall.php` (там `WP_UNINSTALL_PLUGIN`). В `src/` её быть не должно.

2. **Обращение к классу, наследующему класс WooCommerce.** `MethodPickup extends WC_Shipping_Method`: без загруженного WooCommerce его нельзя даже автозагрузить, поэтому `MethodPickup::ID` из юнит-теста роняет PHP. Идентификаторы таких классов вынесены в `Shipping\Methods` — класс без зависимостей от WooCommerce.
...
Боевое хранилище секретов — опции WordPress, они задаются на экране настроек. `.env.local` только удобство локальной разработки: `Install\EnvFile` на бутстрапе заполняет из него **пустые** опции.

Два правила, от которых зависит предсказуемость:

1. **Заданное в админке файлом не перетирается.** Иначе правка ключей через интерфейс молча откатывалась бы к содержимому файла.
2. **Пока `client_id` и `client_secret` заданы, файл не читается вообще.** Проверка идёт по опциям (они автозагружены), поэтому в обычном запросе обращения к диску нет.
