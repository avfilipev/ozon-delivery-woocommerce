...
final class DeliveryPointTest extends TestCase {
	...
	/**
	 * Формы адресов взяты из живой выгрузки каталога (45 363 точки).
	 *
	 * Ozon отдаёт адрес одной строкой, начиная со страны, и глубина
	 * вложенности плавает: город может быть на второй, третьей или
	 * четвёртой позиции. Отдельного поля с городом в API нет — проверено
	 * на живом ответе delivery-point/info.
	 *
	...
	public static function address_provider(): array {
		return array(
			// Город сразу после страны — города федерального значения.
			'Москва'             => array( 'Россия, Москва, Большая Очаковская улица, 10 корпус 1', 'Москва' ),
			'Севастополь'        => array( 'Россия, Севастополь, улица Ленина, 12', 'Севастополь' ),

			// Страна, регион, город.
			'через регион'       => array( 'Россия, Тюменская Область, Тюмень, улица Республики, 204к17', 'Тюмень' ),
			'область кратко'     => array( 'Россия, Московская, Балашиха, улица Свердлова, 16/5', 'Балашиха' ),
			'город из двух слов' => array( 'Россия, Нижегородская, Нижний Новгород, улица имени Генерала Ивлиева И.Д., 33', 'Нижний Новгород' ),

			// Страна, регион, район, населённый пункт.
			'через район'        => array( 'Россия, Пензенская Область, Вадинский Район, Ртищево, улица Плант, 23', 'Ртищево' ),
			'село через район'   => array( 'Россия, Тюменская Область, Тюменский Район, Малые Акияры, улица Туйчина, 2', 'Малые Акияры' ),

			// Другие типы улиц.
			'проспект'           => array( 'Россия, Свердловская Область, Екатеринбург, проспект Ленина, 5', 'Екатеринбург' ),
			'шоссе'              => array( 'Россия, Московская, Химки, Ленинградское шоссе, 1', 'Химки' ),
			'микрорайон'         => array( 'Россия, Московская, Одинцово, микрорайон Трёхгорка, 4', 'Одинцово' ),
			'переулок'           => array( 'Россия, Татарстан, Казань, Овражный переулок, 7', 'Казань' ),

			// Улица без типового слова — работает запасное правило.
			'улица без типа'     => array( 'Россия, Красноярский Край, Канск, Московская, 75', 'Канск' ),

			// Префикс населённого пункта снимается.
			'с префиксом г.'     => array( 'Россия, г. Москва, улица Тверская, 1', 'Москва' ),
	...
	/**
	 * Страна городом быть не может: именно так ломался прежний разбор —
	 * все 45 363 точки получали city = «Россия».
	 */
	public function test_country_is_never_mistaken_for_a_city(): void {
		$point = DeliveryPoint::from_api(
			array(
				'delivery_point_id' => 1,
				'name'              => 'Точка',
				'full_address'      => 'Россия, Московская, Балашиха, улица Свердлова, 16/5',
			)
		);

		self::assertNotSame( 'Россия', $point->city );
	}
