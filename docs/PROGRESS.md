...
1. **Добавить ключи** в WooCommerce → Настройки → Ozon Доставка и нажать «Проверить подключение». Это первый живой прогон OAuth и testcookie. Затем «Обновить каталог» — первый настоящий обход ПВЗ.
...
| 1 | Транспорт, OAuth, testcookie | готово, на живом API не проверялось |
| 2 | Каталог ПВЗ | готово, проверено в wp-env |
| 3 | Метод доставки до ПВЗ | готово, проверено в wp-env |
...
**Ни один запрос к боевому API Ozon не выполнялся ни разу.** Всё проверено юнит-тестами, статическим анализом и прогоном в wp-env с подменой HTTP-слоя через `pre_http_request`.
