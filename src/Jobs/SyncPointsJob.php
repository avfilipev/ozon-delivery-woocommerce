...
final class SyncPointsJob {
	...
	/**
	 * Запускает обход прямо сейчас.
	 *
	 * Уже идущий обход не начинается заново: 45 тысяч точек обходятся больше
	 * часа, и сброс курсора на середине отправил бы всё на второй круг,
	 * удвоив поток запросов к Ozon. Владельцу магазина для этого достаточно
	 * нажать кнопку ещё раз, не дождавшись отклика.
	 *
	 * Шаг ставится в обоих случаях: цепочка задач могла оборваться — упал
	 * Action Scheduler, перезапустили сервер, — и без нового шага «уже идёт»
	 * означало бы «застряло навсегда».
	 *
	 * @return bool Начат ли обход заново. false — продолжен с сохранённого места.
	 */
	public function start_now(): bool {
		$fresh = ! $this->sync->is_running();
